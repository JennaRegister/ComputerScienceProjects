...
//Delete actually works in a similar way to Insert. First you lookup the item. Second, you must reroute references to "ignore" the deleted item. It doesn't actually go anywhere (until it is swept up later), but it will no longer be reported or printed.
	...
	//inserts at the end of the queue (list)
	public void enqueue(AnyType x){
		MyDoubleNode<AnyType> newlink=new MyDoubleNode(tail,tail.prev);//the new link with unique data! It is now pointing AHEAD to head.next and BEHIND to head
		newlink.data=x;//assigning the data into its container (Node)
		(tail.prev).next=newlink;//reroute the "broken" pointers
		tail.prev=newlink;
	}
	//
	public AnyType dequeue(){
		if(!isEmpty()){
			AnyType value = head.next.data;
			if(tail==head){
				tail=null;
			}
			head=head.next;
